...
class Buff2SteamGui(tk.Frame):
	...
	def auth_redis(self, identity):
		redis_service = Redis(host='', port='6379', password='', db=1)
		result = redis_service.get(name=identity).decode()
		if result:
			return True
		return False

	def tips(self):
		# if self.auth_redis('antx'):
		...
		# else:
		# 	messagebox.askquestion(title='鉴权失败', message='鉴权失败，未被授权或授权过期...')
		# 	self.master.destroy()
