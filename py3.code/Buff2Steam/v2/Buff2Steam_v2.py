...
class Buff2SteamGui(tk.Frame):
	...
	def monitor_buff(self, timer: int = 3600):
		...
		while True:
			resp = buff_session.get(url='https://buff.163.com/api/market/steam_trade', headers=header,cookies=cookies).content.decode('utf-8')
	...
	@logger.catch(level='ERROR')
	def auth_quit(self):
		login = tk.Button(self.master, text='鉴权授权', command=self.vauth).place(x=150, y=350)
